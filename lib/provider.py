...
class Result(object):
    ...
    def include_result(self):
        if get_boolean_setting("require_size") and not bool(self._size):
            return False
        if get_boolean_setting("require_seeds") and not bool(self.seeds):
            return False
        if get_boolean_setting("require_resolution"):
            if not get_boolean_setting("include_{}".format(self._resolution.name.lower())):
                return False
        if get_boolean_setting("require_release_type"):
            if not get_boolean_setting("include_{}".format(self._release.name.lower())):
                return False
        return True
